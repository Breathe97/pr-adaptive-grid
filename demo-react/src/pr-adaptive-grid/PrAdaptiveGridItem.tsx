...
export interface PrAdaptiveGridItemProps {
  ...
  dragGeo?: Geo;
  /** 网格滚动容器（坐标换算基准）。
   *  必须显式传入：fixed 定位的 offsetParent 语义在「祖先含 contain/transform」时
   *  会指向该祖先（如 PC 端 shell 的 #app-frame { contain: layout }），而不是 null，
   *  导致按 offsetParent/parentElement 取坐标基准时拿到错误容器 → 回弹终点飞向左上角。 */
...
export function PrAdaptiveGridItem(props: PrAdaptiveGridItemProps) {
  ...
  const geoChanged =
    !!prevGeo &&
    (geo.left !== prevGeo.left ||
      geo.top !== prevGeo.top ||
      geo.width !== prevGeo.width ||
      geo.height !== prevGeo.height);
  ...
  const itemStyle: CSSProperties = (() => {
    ...
    const isFloating = !!floatingGeo || isSettlingAfterDrag;
    return {
  ...
  function readCurrentVisualGeo(): Geo | null {
    ...
    // 坐标基准必须是滚动容器：不能用 offsetParent——fixed item 在「祖先含 contain/layout
    // 或 transform」时 offsetParent 会指向那个祖先（PC 端 shell #app-frame { contain: layout }），
    // 导致内容坐标换算基准错乱，回弹动画终点飞向窗口左上角。
    const parent = (scrollEl ?? outer.parentElement);
  ...
  const toTransform = (newGeo: Geo, options?: TransformOptions) => {
     ...
     *  同上：必须用显式传入的滚动容器，不能信 offsetParent（fixed + contain 祖先时指向错误）。 */
    const parent = (scrollEl ?? outer.parentElement);
    const parentRect = parent?.getBoundingClientRect();
  ...
  useEffect(() => {
    ...
    toTransform(
      effectiveGeo,
      isSettlingRef.current ? { settlingAfterDrag: true } : undefined,
    );
  ...
  useEffect(() => {
    ...
    if (prev !== isSettlingAfterDrag)
      onSettlingChange?.(id, isSettlingAfterDrag);
  }, [isSettlingAfterDrag]);
  ...
  return (
    <div ref={positionRef} className={itemClass} style={itemStyle}>
      <div
        ref={sizeRef}
        className="pr-adaptive-grid-item-size"

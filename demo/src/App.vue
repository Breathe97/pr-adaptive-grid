<template>
  ...
  <div id="app">
    ...
    <div class="grid-wrap">
      ...
      <PrAdaptiveGrid ref="gridRef" :gap="8" @reorder="onReorder">
...
const onReorder = (_payload: GridReorderPayload) => {

  // ids 由组件内部管理，此处可按需监听 list / nextPinId
...
const initGrid = () => {
  const ids = initUsers(userCount.value)
  ids.forEach((id) => ensureItemColor(id))
  gridRef.value?.setItems(
    ids.map((id, index) =>
      index === 0 ? { id, options: { sticky: true, fixed: true } } : { id }
    )
  )
}
